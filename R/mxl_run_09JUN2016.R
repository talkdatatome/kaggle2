...
dataDir <- "/kaggle/data"
driverListFile <- paste(dataDir, "driver_imgs_list.csv", sep='/')
driver_details = read.csv(driverListFile);
...
train_img_matrix = 
  foreach(cls = 0:9, .packages=c('imager'), .combine=rbind, .multicombine=T) %dopar% {
    train_files = list.files(paste0(dataDir, "/train/c", cls, "/"),"*.*",full.names = T);
...
if(TRUE){
  ...
  cl <- makeCluster(cpu.cores); 
  registerDoParallel(cl);
  test_files = data.frame("file"=list.files(dataDir, "/test/","*.*",full.names = T),stringsAsFactors = FALSE);
